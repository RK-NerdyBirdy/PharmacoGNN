...
function createApiClient(o={}){
 ...
 return {request,getToken,clearToken,logout:clearToken,setToken,getSession,getCapability:n=>Boolean(CAPABILITIES[n]),getErrorMessage:e=>e?.message||'Something went wrong. Please try again.',isAuthenticated:()=>Boolean(getToken()),toModelCid:cid,
  register:(email,password,role)=>post('/api/v1/auth/register',{email,password,role}),async login(email,password){const d=await request('/api/v1/auth/login',{method:'POST',auth:false,form:true,body:{username:email,password}});setToken(d.access_token);return d},refresh,getHealth:()=>request('/health',{auth:false}),
